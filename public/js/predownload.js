...
const renderData = (data) => {
    ...
    videoDownloadField.onclick = ()=>{
        downloadFileFromBlob('video', authorUsername + "_" + videoTitle + "_NO_WM", ".mp4")
    }
    videoDownloadWMField.onclick = ()=>{
        downloadFileFromBlob('video_wm', authorUsername + "_" + videoTitle + "_WM", ".mp4")
    }
    musicDownloadField.onclick = ()=>{
        downloadFileFromBlob('music', musicAuthor + "_" + musicTitle, ".mp3")
    }
...
const getBlob = (blobKey, url) => {
    ...
    xhr.onreadystatechange = function() {
        if (this.readyState === this.DONE) {
            ...
            console.log("setting blob " + blobKey)
            BLOBS[blobKey] = blob
